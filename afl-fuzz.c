...
static u8  is_eff = 0;                /*@BBL eff_map是否已经分配内存         */
...
static u64 total_nfuzz;               /*@BBL 所有input的ｎ_fuzz的总和        */
static u32 total_input_num;           /*@BBL input队列的大小                */
...
struct queue_entry {
  ...
  u64 exec_us,                        /* Execution time (us)              */
      ...
      depth,                          /* Path depth                       */
      n_fuzz,                         /* @BBL Number of fuzz, does not overflow */
      num_eff_mutate,                 /* @BBL Number of effective mutation*/
      num_mutate;                     /* @BBL Number of all mutation      */
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    queue_cur->num_eff_mutate++;      //@BBL 计算有效变异个数
...
static void show_stats(void) {
  ...
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
          ...
          cYEL "american fuzzy lop（hare）", use_banner);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  queue_cur->num_mutate++;    //@BBL 队列中某一个input的变异总次数累加
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  u32 avg_nfuzz = total_nfuzz / total_input_num;  //@BBL
  u32 perf_score = 100;
  ...
  if(avg_nfuzz != 0){
    ...
    else if(cmp_nfuzz < 1 && q->n_fuzz != 0){
      cmp_nfuzz = avg_nfuzz / q->n_fuzz;
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  if(len > 0){
    ...
    else if(eff_cnt * 100 / len > 1)
      queue_cur->eff_score = 3;
    else if(eff_cnt * 1000 / len > 1)
      queue_cur->eff_score = 2;
    else
      queue_cur->eff_score = 1;
  }
  else{
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
        ...
        case 10:{
            ...
            if((is_eff && eff_map[EFF_APOS(pos)]) || UR(100) > HAVOC_RAND){
